...
describe Contacts::Yahoo do
  ...
  it 'should have a simple interface to grab the contacts' do
    @yahoo.should_receive(:access_user_credentials).and_return(read_file('yh_credential.xml'))
    @yahoo.should_receive(:access_address_book_api).and_return(read_file('yh_contacts.txt'))
